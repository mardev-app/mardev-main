import { useState, useEffect } from 'react';
import { Button } from '@/components/ui/button';
import { LogOut, User, LogIn, WifiOff, Sparkles } from 'lucide-react';
import { useAuth } from '@/contexts/AuthContext';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  DropdownMenuTrigger,
} from '@/components/ui/dropdown-menu';
import { useNavigate } from 'react-router-dom';

export const AuthStatus = () => {
  const { user, loading, signIn, signOut, isOnboardingComplete, isOfflineMode } = useAuth();
  const [isSigningIn, setIsSigningIn] = useState(false);
  const [authTimeout, setAuthTimeout] = useState(false);
  const navigate = useNavigate();

  // Add timeout for auth loading
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (loading) {
        console.log('Auth loading timeout, showing fallback');
        setAuthTimeout(true);
      }
    }, 8000); // Reduced from 15 seconds to 8 seconds

    return () => clearTimeout(timeoutId);
  }, [loading]);

  const handleSignIn = async (provider: 'google' | 'github' | 'discord') => {
    try {
      setIsSigningIn(true);
      await signIn(provider);
    } catch (error: any) {
      console.error('Sign in error:', error);
      // If OAuth providers are not configured, show a helpful message
      if (error.message?.includes('provider is not enabled')) {
        alert('OAuth providers are not configured yet. Please contact the administrator.');
      } else {
        alert('Authentication failed. Please try again.');
      }
      setIsSigningIn(false);
    }
  };

  const handleSignOut = async () => {
    try {
      console.log('Sign out button clicked');
      await signOut();
    } catch (error) {
      console.error('Sign out error in component:', error);
      // Force reload on error as fallback
      window.location.reload();
    }
  };

  const handleOnboarding = () => {
    console.log('Onboarding button clicked');
    navigate('/onboarding');
  };

  const handleOfflineMode = () => {
    // In offline mode, we can simulate a logged-in state for testing
    // This is just for development/testing purposes
    if (confirm('Enable offline demo mode? This will simulate a logged-in user for testing.')) {
      // This would need to be implemented in the AuthContext
      // For now, just show a message
      alert('Offline demo mode would be implemented here for testing purposes.');
    }
  };

  // Show offline mode indicator
  if (isOfflineMode) {
    return (
      <div className="fixed top-6 right-6 z-50 flex items-center gap-3">
        <div className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 backdrop-blur-xl border border-yellow-500/30 rounded-xl px-4 py-2 flex items-center gap-2 shadow-lg">
          <WifiOff className="w-4 h-4 text-yellow-400" />
          <span className="text-sm text-yellow-400 font-medium">Offline Mode</span>
        </div>
        <Button 
          variant="outline" 
          size="sm" 
          onClick={handleOfflineMode}
          className="border-blue-500/50 text-blue-400 hover:bg-blue-500/20 hover:text-blue-300 backdrop-blur-xl"
        >
          Demo Mode
        </Button>
      </div>
    );
  }

  // Show fallback if auth is taking too long
  if (loading && authTimeout) {
    return (
      <div className="fixed top-6 right-6 z-50">
        <div className="bg-gradient-to-r from-yellow-500/20 to-orange-500/20 backdrop-blur-xl border border-yellow-500/30 rounded-xl px-4 py-2 shadow-lg">
          <span className="text-sm text-yellow-400 font-medium">Auth system unavailable</span>
        </div>
      </div>
    );
  }

  // If we have a timeout but still loading, show a fallback with refresh option
  if (loading && authTimeout) {
    return (
      <div className="fixed top-6 right-6 z-[9999] pointer-events-auto">
        <div className="bg-gradient-to-r from-orange-500/20 to-red-500/20 backdrop-blur-xl border border-orange-500/30 rounded-xl px-4 py-2 shadow-lg">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-orange-400 rounded-full animate-pulse"></div>
            <span className="text-sm text-orange-400 font-medium">Auth timeout</span>
            <Button 
              size="sm" 
              variant="ghost" 
              onClick={() => window.location.reload()}
              className="h-6 px-2 text-xs text-orange-400 hover:bg-orange-500/20"
            >
              Refresh
            </Button>
          </div>
        </div>
      </div>
    );
  }

  if (loading && !authTimeout) {
    return (
      <div className="fixed top-6 right-6 z-[9999] pointer-events-auto">
        <div className="bg-gradient-to-r from-blue-500/20 to-purple-500/20 backdrop-blur-xl border border-blue-500/30 rounded-xl px-4 py-2 shadow-lg">
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 bg-blue-400 rounded-full animate-pulse"></div>
            <span className="text-sm text-blue-400 font-medium">Checking auth...</span>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="fixed top-6 right-6 z-[9999] flex items-center gap-3 pointer-events-auto">
      {user ? (
        <>
          <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 backdrop-blur-xl border border-green-500/30 rounded-xl px-4 py-2 flex items-center gap-2 shadow-lg">
            <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
            <User className="w-4 h-4 text-green-400" />
            <span className="text-sm text-green-400 font-medium">
              {isOnboardingComplete ? 
                (user.user_metadata?.name || user.user_metadata?.display_name || user.email || 'Welcome back!') :
                (user.email || 'Welcome back!')
              }
            </span>
          </div>
          {!isOnboardingComplete && (
            <Button 
              variant="outline" 
              size="sm" 
              onClick={handleOnboarding}
              className="border-yellow-500/50 text-yellow-400 hover:bg-yellow-500/20 hover:text-yellow-300 backdrop-blur-xl pointer-events-auto cursor-pointer"
              style={{ zIndex: 9999 }}
            >
              <Sparkles className="w-4 h-4 mr-2" />
              Complete Setup
            </Button>
          )}
          <Button 
            variant="outline" 
            size="sm" 
            onClick={handleSignOut}
            className="border-red-500/50 text-red-400 hover:bg-red-500/20 hover:text-red-300 backdrop-blur-xl pointer-events-auto cursor-pointer"
            style={{ zIndex: 9999 }}
          >
            <LogOut className="w-4 h-4" />
            Logout
          </Button>
        </>
      ) : (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button 
              variant="default" 
              size="sm" 
              disabled={isSigningIn}
              className="bg-gradient-primary hover:opacity-90 text-white font-medium shadow-lg backdrop-blur-xl"
            >
              <LogIn className="w-4 h-4 mr-2" />
              {isSigningIn ? 'Signing in...' : 'Login'}
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-56 bg-black/95 backdrop-blur-xl border-gray-800 shadow-xl">
            <DropdownMenuItem onClick={() => handleSignIn('google')} className="cursor-pointer hover:bg-gray-800 text-white focus:bg-gray-800 focus:text-white">
              <svg className="w-4 h-4 mr-2" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
              Continue with Google
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleSignIn('github')} className="cursor-pointer hover:bg-gray-800 text-white focus:bg-gray-800 focus:text-white">
              <svg className="w-4 h-4 mr-2" viewBox="0 0 24 24">
                <path fill="currentColor" d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
              </svg>
              Continue with GitHub
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => handleSignIn('discord')} className="cursor-pointer hover:bg-gray-800 text-white focus:bg-gray-800 focus:text-white">
              <svg className="w-4 h-4 mr-2" viewBox="0 0 24 24">
                <path fill="#5865F2" d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419-.019 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1568 2.4189Z"/>
              </svg>
              Continue with Discord
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      )}
    </div>
  );
};